...
class HikingTraversable extends Traversable<number> {
  ...
  validTrailEndLocations: [number, number][] = [];
  ...
  findTrails() {
    ...
    for (const _idx in trailheads) {
      ...
      this.findValidPaths([0], trailheadLoc);
      console.log(
  ...
  findValidPaths(numsSoFar: number[], location: [number, number]) {
    this.goTo(...location);
    ...
    peeks.forEach(([direction, { location: peekLoc, value }]) => {
      ...
      if (value === 9 && numsSoFar.length === 9) {
        this.validTrails++;
        return;
      }
      if (value === lastNum + 1) {
        this.findValidPaths([...numsSoFar, value], peekLoc);
      }
    });
  }
